...
export function validateTheme(theme) {
    ...
    for (const field of ['id', 'name', 'version']) {
        if (typeof t[field] !== 'string' || !t[field].trim()) {
            throw new ThemeValidationError(`Theme field "${field}" must be a non-empty string.`);
        }
    }
    // id format: "vendor/theme-name"
    const id = t['id'];
    if (!/^[a-z0-9-]+\/[a-z0-9-]+$/.test(id)) {
        throw new ThemeValidationError(`Theme id "${id}" must follow the format "vendor/theme-name" (lowercase alphanumeric and hyphens).`);
    }
    ...
    for (const colorKey of [
        ...
        if (typeof colors[colorKey] !== 'string') {
            throw new ThemeValidationError(`Theme tokens.colors.${colorKey} must be a string.`);
        }
    }
    ...
    for (const textKey of ['primary', 'secondary', 'muted']) {
        if (typeof textColors[textKey] !== 'string') {
            throw new ThemeValidationError(`Theme tokens.colors.text.${textKey} must be a string.`);
        }
    }
    ...
    if (!typography['fontFamily'] ||
        ...
        throw new ThemeValidationError('Theme tokens.typography.fontFamily must be an object.');
    }
    const fontFamily = typography['fontFamily'];
    for (const fk of ['sans', 'mono']) {
        if (typeof fontFamily[fk] !== 'string') {
            throw new ThemeValidationError(`Theme tokens.typography.fontFamily.${fk} must be a string.`);
        }
    }
    for (const recordField of ['fontSize', 'fontWeight', 'lineHeight', 'spacing', 'radius', 'shadows']) {
        ...
        if (!target || typeof target !== 'object' || Array.isArray(target)) {
            throw new ThemeValidationError(`Theme tokens.${recordField} must be a key-value object.`);
        }
    }
    ...
    for (const ak of ['duration', 'easing']) {
        if (typeof animation[ak] !== 'string') {
            throw new ThemeValidationError(`Theme tokens.animation.${ak} must be a string.`);
        }
    }
}
// ---------------------------------------------------------------------------
...
function injectCssVars(vars) {
    ...
    if (typeof document !== 'undefined') {
        let styleEl = document.getElementById('volqan-theme-tokens');
        if (!styleEl) {
...
export function loadTheme(theme) {
    ...
    if (themeRegistry.has(theme.id)) {
        console.warn(`[volqan/themes] Theme "${theme.id}" is already registered — overwriting.`);
    }
...
export function applyTheme(themeId) {
    ...
    if (!theme) {
        throw new Error(`[volqan/themes] Cannot apply theme "${themeId}" — it has not been loaded. ` +
 ...
 */
export function loadAndApplyTheme(theme) {
 ...
 */
export function unloadTheme(themeId) {
 ...
 */
export function getComponentOverride(componentName) {
 ...
 */
export function mergeComponentStyle(componentName, baseStyle = {}) {
    const override = getComponentOverride(componentName);
    if (!override?.cssVars)
        return baseStyle;
    return { ...baseStyle, ...override.cssVars };
}
