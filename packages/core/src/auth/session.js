 ...
 */
function expiresFromNow(ttlSeconds) {
    return new Date(Date.now() + ttlSeconds * 1000);
}
/**
 * Maps a Prisma User + Session record to an {@link AuthSession}.
 */
function mapToAuthSession(session) {
    const user = {
        id: session.user.id,
...
export async function createSession(options) {
    const { userId, ipAddress = null, userAgent = null, ttlSeconds = DEFAULT_SESSION_TTL_SECONDS, } = options;
 ...
 */
export async function refreshSession(token, ttlSeconds = DEFAULT_SESSION_TTL_SECONDS) {
