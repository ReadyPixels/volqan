...
export async function requireRole(request, role) {
    ...
    if (!hasRole(auth.user.role, role)) {
        throw new AuthError('INSUFFICIENT_PERMISSIONS', `This action requires the "${role}" role or higher. You have "${auth.user.role}".`, 403);
    }
    return auth;
}
/**
 ...
 */
export function setSessionCookie(response, options) {
