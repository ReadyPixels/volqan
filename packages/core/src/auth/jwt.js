...
function getJwtSecret() {
    ...
    if (!secret) {
        if (process.env['NODE_ENV'] === 'production') {
            throw new Error('[volqan:jwt] JWT_SECRET environment variable is required in production.');
        }
        // Development fallback — NOT safe for production
        console.warn('[volqan:jwt] ⚠  JWT_SECRET is not set — using insecure development fallback.');
        return 'volqan.link-secret-do-not-use-in-production-please-set-jwt-secret';
    }
    if (process.env['NODE_ENV'] === 'production' && secret.length < 32) {
        throw new Error('[volqan:jwt] JWT_SECRET must be at least 32 characters long.');
    }
    return secret;
}
let _jwt = null;
async function getJwt() {
    ...
    try {
        ...
        throw new Error('[volqan:jwt] jsonwebtoken is not installed. Run: pnpm add jsonwebtoken');
    }
}
// ---------------------------------------------------------------------------
// Token generation
...
async function signToken(payload, expiresIn) {
    ...
    return new Promise((resolve, reject) => {
        jwt.sign(payload, secret, { algorithm: JWT_ALGORITHM, expiresIn }, (err, token) => {
            if (err || !token) {
