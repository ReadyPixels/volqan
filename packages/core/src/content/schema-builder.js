 ...
 */
import { FieldType, ContentValidationError, ContentTypeNotFoundError, } from './types.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
export class SchemaBuilder {
     ...
     */
    async updateContentType(slug, updates) {
     ...
     */
    async deleteContentType(slug, deleteEntries = false) {
        const existing = await this.prisma.contentType.findUnique({ where: { slug } });
        if (!existing)
            throw new ContentTypeNotFoundError(slug);
        if (deleteEntries) {
     ...
     */
    validateEntry(contentType, data) {
        const errors = [];
