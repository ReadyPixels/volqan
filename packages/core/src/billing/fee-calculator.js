 ...
 */
export function calculateServiceFee(baseCents, isPayPal) {
    if (!Number.isFinite(baseCents) || baseCents < 0) {
        throw new RangeError(`calculateServiceFee: baseCents must be a non-negative finite number. Received: ${baseCents}`);
    }
 ...
 */
export function calculateMonthlyPrice(yearlyPrice) {
    if (!Number.isFinite(yearlyPrice) || yearlyPrice < 0) {
        throw new RangeError(`calculateMonthlyPrice: yearlyPrice must be a non-negative finite number. Received: ${yearlyPrice}`);
    }
 ...
 */
export function calculateBuyerTotal(listingPriceCents, isPayPal) {
 ...
 */
export function calculateSellerPayout(listingPriceCents) {
    if (!Number.isFinite(listingPriceCents) || listingPriceCents < 0) {
        throw new RangeError(`calculateSellerPayout: listingPriceCents must be a non-negative finite number. Received: ${listingPriceCents}`);
    }
 ...
 */
export function calculatePlatformRevenue(listingPriceCents, isPayPal) {
 ...
 */
export function isValidListingPrice(priceCents) {
    return (Number.isFinite(priceCents) &&
 ...
 * // }
 */
export function getDetailedFeeBreakdown(listingPriceCents, isPayPal) {
    if (!Number.isFinite(listingPriceCents) || listingPriceCents < 0) {
        throw new RangeError(`getDetailedFeeBreakdown: listingPriceCents must be a non-negative finite number.`);
    }
    ...
    const totalServiceFeeCents = FLAT_FEE_CENTS + variableFeeCents + paypalSurchargeCents;
    const buyerTotalCents = listingPriceCents + totalServiceFeeCents;
    const sellerPayoutCents = Math.floor(listingPriceCents * 0.70);
    const platformRevenueCents = listingPriceCents - sellerPayoutCents + totalServiceFeeCents;
    return {
        listingPriceCents,
