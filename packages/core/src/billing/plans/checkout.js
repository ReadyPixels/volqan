 ...
 */
export async function createCheckoutSession(stripe, options) {
    const { userId, planId, stripePriceId, planPriceCents, successUrl, cancelUrl, customerEmail, installationId, stripeCustomerId, } = options;
    if (!stripePriceId) {
        throw new Error(`[volqan/checkout] No Stripe Price ID configured for plan "${planId}". ` +
        ...
        line_items: [
            ...
            // Recurring fees are recorded via webhook after each renewal.
            {
                price_data: {
                    ...
                    product_data: {
                        name: 'Service Fee',
                        description: 'Platform Service Fee — $0.50 flat + 10% of plan price. ' +
                            'See volqan.link/terms for full fee disclosure.',
                    },
                    unit_amount: serviceFeeCents,
                    recurring: {
                        interval: planId === 'support-yearly' ? 'year' : 'month',
                    },
                },
                quantity: 1,
            },
    ...
    if (!session.url) {
        throw new Error('[volqan/checkout] Stripe returned a checkout session without a URL.');
    }
    return {
        url: session.url,
 ...
 */
export async function createCustomerPortalSession(stripe, stripeCustomerId, returnUrl) {
