...
export const SUPPORT_PLAN_FEATURES = [
 ...
 */
export function buildSupportYearlyPlan(yearlyPriceCents, stripePriceId) {
    return {
        id: PLAN_ID_SUPPORT_YEARLY,
        name: 'Support Plan — Yearly',
        description: 'Annual subscription. Best value — save 17% compared to monthly billing.',
        interval: 'yearly',
 ...
 */
export function buildSupportMonthlyPlan(yearlyPriceCents, stripePriceId) {
    return {
 ...
 */
export function buildPlans(yearlyPriceCents, yearlyStripePriceId, monthlyStripePriceId) {
    return {
 ...
 */
export function calculatePrice(yearlyPriceCents, interval) {
    if (interval === 'yearly') {
