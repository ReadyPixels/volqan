 ...
 */
import { invalidateLicenseCache, seedLicenseCache } from '../license/checker.js';
import { activateSubscription, handleRenewal, handlePaymentFailed, handleCancellation, getSubscriptionByStripeId, recordInvoice, } from './plans/subscription-manager.js';
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
 ...
 * ```
 */
export function createWebhookHandler(stripeOrOptions, legacyLicenseStore) {
    ...
    return async function handleStripeWebhook(req) {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : String(err);
            console.error(`[volqan/billing] Error handling Stripe event "${event.type}" (${event.id}): ${message}`);
...
// Event router
// ---------------------------------------------------------------------------
async function routeStripeEvent(event, store, subscriptionStore, emailer) {
    switch (event.type) {
        case 'checkout.session.completed':
            await handleCheckoutSessionCompleted(event.data.object, store, subscriptionStore, emailer);
            break;
        case 'customer.subscription.created':
            await handleSubscriptionCreated(event.data.object, store, subscriptionStore, emailer);
            break;
        case 'customer.subscription.updated':
            await handleSubscriptionUpdated(event.data.object, store, subscriptionStore);
            break;
        case 'customer.subscription.deleted':
            await handleSubscriptionDeleted(event.data.object, store, subscriptionStore);
            break;
        case 'customer.subscription.paused':
            await handleSubscriptionPaused(event.data.object, store, subscriptionStore);
            break;
        case 'customer.subscription.resumed':
            await handleSubscriptionResumed(event.data.object, store, subscriptionStore);
            break;
        case 'invoice.payment_failed':
            await handleInvoicePaymentFailed(event.data.object, store, subscriptionStore, emailer);
            break;
        case 'invoice.payment_succeeded':
            await handleInvoicePaymentSucceeded(event.data.object, store, subscriptionStore);
            break;
        default:
 ...
 */
async function handleCheckoutSessionCompleted(session, store, subscriptionStore, emailer) {
    ...
    if (!stripeSubscriptionId) {
        console.warn('[volqan/billing] checkout.session.completed: no subscription ID on session');
        return;
    }
    const userId = session.metadata?.['userId'] ?? session.client_reference_id ?? '';
    const planId = session.metadata?.['planId'] ?? 'support-yearly';
    console.info(`[volqan/billing] Checkout completed for user ${userId} (plan: ${planId})`);
 ...
 */
async function handleSubscriptionCreated(subscription, store, subscriptionStore, emailer) {
    ...
    console.info(`[volqan/billing] Activating license for customer ${customerId} (plan: ${plan})`);
    const { installationId } = await store.activateLicense(customerId, subscription.id, plan, expiresAt);
 ...
 */
async function handleSubscriptionUpdated(subscription, store, subscriptionStore) {
    const customerId = resolveCustomerId(subscription.customer);
    const plan = resolveInterval(subscription);
    const expiresAt = resolveExpiresAt(subscription);
    const isActive = subscription.status === 'active';
    console.info(`[volqan/billing] Refreshing license for customer ${customerId} (status: ${subscription.status})`);
    const { installationId } = await store.refreshLicense(customerId, subscription.id, plan, expiresAt);
    // Update subscription record
    if (subscriptionStore) {
        const periodStart = new Date(subscription.current_period_start * 1000);
        const periodEnd = new Date(subscription.current_period_end * 1000);
        if (subscription.status === 'active') {
            await handleRenewal(subscriptionStore, subscription.id, periodStart, periodEnd).catch((err) => {
                console.warn(`[volqan/billing] Could not update renewal dates: ${String(err)}`);
            });
        }
        else {
            await subscriptionStore
                ...
                .catch((err) => {
                console.warn(`[volqan/billing] Could not update subscription status: ${String(err)}`);
            });
        }
    }
    await updateLicenseCache(installationId, {
 ...
 */
async function handleSubscriptionDeleted(subscription, store, subscriptionStore) {
    const customerId = resolveCustomerId(subscription.customer);
    console.info(`[volqan/billing] Revoking license for customer ${customerId}`);
    const { installationId } = await store.revokeLicense(customerId, subscription.id);
    // Update subscription record
    if (subscriptionStore) {
        await handleCancellation(subscriptionStore, subscription.id).catch((err) => {
            console.warn(`[volqan/billing] Could not cancel subscription record: ${String(err)}`);
        });
    }
    await updateLicenseCache(installationId, {
 ...
 */
async function handleSubscriptionPaused(subscription, store, subscriptionStore) {
    const customerId = resolveCustomerId(subscription.customer);
    const plan = resolveInterval(subscription);
    const expiresAt = resolveExpiresAt(subscription);
    console.info(`[volqan/billing] Subscription paused for customer ${customerId}. Pausing attribution removal.`);
    const { installationId } = await store.refreshLicense(customerId, subscription.id, plan, expiresAt);
    // Update subscription record
    if (subscriptionStore) {
        await subscriptionStore
            ...
            .catch((err) => {
            console.warn(`[volqan/billing] Could not update paused status: ${String(err)}`);
        });
    }
    await updateLicenseCache(installationId, {
        attributionRemoved: false,
 ...
 */
async function handleSubscriptionResumed(subscription, store, subscriptionStore) {
    const customerId = resolveCustomerId(subscription.customer);
    const plan = resolveInterval(subscription);
    const expiresAt = resolveExpiresAt(subscription);
    console.info(`[volqan/billing] Subscription resumed for customer ${customerId}. Restoring attribution removal.`);
    const { installationId } = await store.refreshLicense(customerId, subscription.id, plan, expiresAt);
    // Update subscription record
    if (subscriptionStore) {
        const periodEnd = new Date(subscription.current_period_end * 1000);
        const periodStart = new Date(subscription.current_period_start * 1000);
        await handleRenewal(subscriptionStore, subscription.id, periodStart, periodEnd).catch((err) => {
            console.warn(`[volqan/billing] Could not update resumed subscription: ${String(err)}`);
        });
    }
    await updateLicenseCache(installationId, {
        attributionRemoved: true,
 ...
 */
async function handleInvoicePaymentFailed(invoice, store, subscriptionStore, emailer) {
    const customerId = resolveInvoiceCustomerId(invoice);
    console.warn(`[volqan/billing] Payment failed for customer ${customerId}. Starting ${GRACE_PERIOD_DAYS}-day grace period.`);
    const { installationId } = await store.startGracePeriod(customerId, invoice.id, GRACE_PERIOD_DAYS);
    ...
    if (subscriptionStore && stripeSubscriptionId) {
        const sub = await subscriptionStore
            ...
            .catch(() => null);
        if (sub) {
            await handlePaymentFailed(subscriptionStore, stripeSubscriptionId).catch((err) => {
                console.warn(`[volqan/billing] Could not mark subscription as past_due: ${String(err)}`);
            });
            // Send warning email
            if (emailer) {
                sendEmailSafely(() => emailer.sendPaymentFailedEmail(sub.userId, GRACE_PERIOD_DAYS));
            }
        }
    }
 ...
 */
async function handleInvoicePaymentSucceeded(invoice, store, subscriptionStore) {
    const customerId = resolveInvoiceCustomerId(invoice);
    console.info(`[volqan/billing] Payment succeeded for customer ${customerId}. Extending license.`);
    // Determine new expiry from invoice line items (period end of first item)
    const expiresAt = resolveInvoiceExpiresAt(invoice);
    const { installationId } = await store.extendLicense(customerId, invoice.id, expiresAt);
        ...
        if (stripeSubscriptionId) {
            ...
            if (sub) {
                ...
                await recordInvoice(subscriptionStore, sub.id, {
                    ...
                    console.warn(`[volqan/billing] Could not record invoice: ${String(err)}`);
                });
            }
        }
    }
 ...
 */
async function updateLicenseCache(installationId, status) {
    ...
    console.info(`[volqan/billing] License cache updated for installation "${installationId}": ` +
 ...
 */
function buildInvoiceItems(invoice) {
 ...
 */
function resolveServiceFeeFromInvoice(invoice) {
    const feeLine = (invoice.lines?.data ?? []).find((line) => line.description?.toLowerCase().includes('service fee'));
...
function resolveInvoiceCustomerId(invoice) {
...
function resolveExpiresAt(subscription) {
    if (!subscription.current_period_end)
        return null;
    return new Date(subscription.current_period_end * 1_000);
}
function resolveInvoiceExpiresAt(invoice) {
