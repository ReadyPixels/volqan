 ...
 * ```
 */
export function buildMarketplaceURL(type, filters) {
    const base = type === 'extension'
 ...
 * ```
 */
export function parseInstallURL(url) {
    try {
        ...
        const slug = decodeURIComponent(parsed.pathname.slice(installPrefix.length)).replace(/\/+$/, '');
