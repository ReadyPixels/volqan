...
export class ImageProcessor {
    ...
    async generateThumbnail(input, options = {}) {
        const sharp = await this._loadSharp();
        const { width = 300, height = 300, fit = 'cover', format = 'webp', quality = 80, } = options;
    ...
    async resizeImage(input, options) {
        ...
        const sharp = await this._loadSharp();
        const { width, height, fit = 'inside', quality = 85, withoutEnlargement = true, } = options;
     ...
     */
    async convertFormat(input, format, quality = 85) {
        const sharp = await this._loadSharp();
        const pipeline = sharp(input);
        switch (format) {
            case 'jpeg':
    ...
    async _loadSharp() {
        try {
            ...
            throw new Error('ImageProcessor requires the "sharp" package to be installed. ' +
                'Run: pnpm add sharp');
        }
    }
}
