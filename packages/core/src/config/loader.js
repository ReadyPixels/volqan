    ...
    for (const candidate of candidates) {
        if (existsSync(candidate)) {
            try {
                ...
                console.warn(`[volqan:config] Failed to load config file at ${candidate}:`, err);
            }
        }
    }
    return {};
...
function loadEnvOverrides() {
    ...
    if (env['UPLOAD_DIR']) {
        override.storage = {
            ...override.storage,
            local: { ...override.storage?.local, path: env['UPLOAD_DIR'] },
        };
    }
    if (env['UPLOAD_PUBLIC_URL']) {
        override.storage = {
            ...override.storage,
            local: { ...override.storage?.local, publicUrl: env['UPLOAD_PUBLIC_URL'] },
        };
    }
...
export function getConfig() {
    if (!_resolvedConfig) {
        throw new Error('[volqan:config] Configuration has not been loaded yet. ' +
...
function validateConfig(config) {
    if (process.env['NODE_ENV'] === 'production') {
        if (config.auth.jwtSecret ===
            ...
            throw new Error('[volqan:config] JWT_SECRET must be set in production. ' +
                'Set the JWT_SECRET environment variable or auth.jwtSecret in volqan.config.ts.');
        }
        if (config.auth.jwtSecret.length < 32) {
            throw new Error('[volqan:config] auth.jwtSecret must be at least 32 characters in production.');
        }
    }
    if (!config.database.url) {
        throw new Error('[volqan:config] database.url must be set. ' +
