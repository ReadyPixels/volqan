 ...
 */
export async function checkLicenseStatus(installationIdOverride) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.warn(`[volqan/license] License API returned HTTP ${response.status}. Attribution will be shown.`);
            const fallback = { attributionRemoved: false };
        ...
        console.warn(`[volqan/license] License check failed (${String(err)}). Showing attribution as fallback.`);
 ...
 */
export async function invalidateLicenseCache(installId) {
 ...
 */
export async function seedLicenseCache(status, installId, ttlMs) {
    const id = installId ?? (await getInstallationId());
...
function normalisePlan(plan) {
    if (!plan)
        return undefined;
    if (plan.includes('yearly') || plan.includes('year'))
        return 'yearly';
